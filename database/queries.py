...
async def get_expiring_keys(days: int = 3) -> List[Dict]:
    """
...
async def update_key_server(key_id: int, server_id: int) -> None:
    """
    Обновление сервера для ключа.
    
    Args:
        key_id: ID ключа
        server_id: Новый ID сервера
    """
    await db.execute(
        "UPDATE user_keys SET server_id = ? WHERE id = ?",
        (server_id, key_id)
    )
