...
async def show_tariffs(message: Message, db_user: dict = None):
    ...
    if not tariffs:
        await message.answer(
            ...
            "Обратитесь в поддержку: @FreakVPN_Support"
        )
        return
...
async def callback_buy_tariff(callback: CallbackQuery, db_user: dict = None):
    ...
    savings = calculate_savings(tariff["months"], price)
...
def calculate_savings(months: int, price: int) -> int:
    """
    ...
    """
    # Базовая цена за 1 месяц (299 ₽ = 29900 копеек)
    base_monthly_price = 29900
    
    if months == 1:
        return 0
    
    # Расчет ожидаемой цены без скидки
    expected_price = base_monthly_price * months
