...
async def process_promo_code(message: Message, state: FSMContext):
    ...
    Этот хендлер должен быть в отдельном FSM обработчике,
    но для простоты добавлен здесь.
    """
    # Проверяем текущее состояние
    current_state = await state.get_state()
    
    if current_state and "waiting_promo" in str(current_state):
        promo_code = message.text.strip().upper()
        
        # TODO: Реализовать проверку промокода
        await message.answer(
            "❌ Промокод не найден или уже использован.\n\n"
            "Попробуй другой или напиши в поддержку."
        )
