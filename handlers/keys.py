...
async def callback_confirm_change_key(callback: CallbackQuery, db_user: dict = None):
    ...
    try:
        # Шаг 1: Сначала создаём новый ключ на новом сервере
        ...
        xui_new = XuiService(new_server)
        new_key = await xui_new.create_client(user_id, days=days_left)
        
        if not new_key:
            await callback.answer("❌ Ошибка при создании нового ключа", show_alert=True)
            return
        
        # Увеличиваем нагрузку нового сервера
        await queries.increment_server_load(new_server["id"])
        
        # Шаг 2: Только после успешного создания нового ключа - удаляем старый
        ...
        # Шаг 3: Обновляем записи в БД (удаляем старый, создаём новый)
        ...
        await queries.create_user_key(
            ...
            auto_renew=key_data["auto_renew"]
        )
        
        await callback.message.edit_text(
            f"✅ Ключ успешно изменен!\n\n🌍 Новый сервер: {new_server['name']}\n🔑 Новый ключ:\n<code>{new_key['key']}</code>",
            reply_markup=get_back_and_close_keyboard("go_to_keys")
        )
