...
def get_buy_keyboard(tariffs: List[Dict], has_trial: bool = True, extend_key_id: int = None) -> InlineKeyboardMarkup:
    """
    Клавиатура раздела 'Купить' по ТЗ StarinaVPN.
    ...
    # Платные тарифы с callback по ТЗ: buy_7days, buy_1month, buy_3months, buy_6months, buy_12months
    for tariff in tariffs:
        if tariff["price"] == 0:
            continue  # Пробный уже показали
        price_rub = tariff["price"] // 100
        
        # Формируем callback_data по ТЗ
        if extend_key_id:
            # Для продления используем старый формат
            callback = f"extend_tariff_{tariff['id']}_{extend_key_id}"
        else:
            # Для покупки используем формат по ТЗ
            callback = get_tariff_callback(tariff)
...
def get_tariff_callback(tariff: Dict) -> str:
    """
    Получение callback для тарифа по ТЗ.
    
    Args:
        tariff: Данные тарифа
        
    Returns:
        callback_data строка
    """
    days = tariff.get("days", 0)
    months = tariff.get("months", 0)
    
    if months == 0 and days == 7:
        return "buy_7days"
    elif months == 1:
        return "buy_1month"
    elif months == 3:
    ...
    # Fallback для нестандартных тарифов
    return f"buy_tariff_{tariff['id']}"
...
def get_main_keyboard() -> InlineKeyboardMarkup:
    """
    Reply-клавиатура главного меню (StarinaVPN).
    Кнопки без эмодзи по ТЗ.
    
    Структура:
    Строка 1: Купить
    Строка 2: Мои ключи Профиль
    Строка 3: Поддержка Отзывы
    Строка 4: Промокод Статус серверов
    """
