...
class SchedulerService:
    ...
    def start(self):
        ...
        # Уведомление за 3 дня до автосписания в 10:00
        self.scheduler.add_job(
            self.notify_autorenew_warning,
            CronTrigger(hour=10, minute=0, timezone=config.TIMEZONE),
            id="notify_autorenew_warning",
            replace_existing=True
        )
    ...
    async def notify_expiring_soon(self):
        ...
        try:
            # Получаем ключи, которые истекают через 3 дня (72 часа)
            expiring_keys = await queries.get_expiring_keys(hours=72)
            
            if not expiring_keys:
                logger.info("Нет ключей с истекающей подпиской")
                return
            ...
            for key in expiring_keys:
                try:
                    ...
                    await self._bot.send_message(
                        key["user_id"],
                        f"Твой ключ истекает через {days_left} дня. Зайди в раздел 'Мои ключи' и продли."
                    )
                    sent += 1
            
                except Exception as e:
                    logger.error(
                        f"Ошибка при отправке уведомления user_id={key['user_id']}: {e}"
                    )
    ...
    async def notify_autorenew_warning(self):
        """
        Уведомление за 3 дня до автоматического списания за продление ключа.
        """
        logger.info("Запуск уведомлений о предстоящем автосписании")
        ...
        try:
            keys = await queries.get_keys_for_auto_renew()
            
            if not keys:
                logger.info("Нет ключей с автопродлением")
                return
            ...
            for key in keys:
                try:
                    ...
                    if days_left == 3:
                        await self._bot.send_message(
                            key["user_id"],
                            f"Через 3 дня автоматически спишутся деньги за продление ключа. Чтобы отключить автопродление, зайди в раздел 'Мои ключи'."
                        )
                        sent += 1
            ...
            logger.info(f"Отправлено {sent} уведомлений о автосписании")
